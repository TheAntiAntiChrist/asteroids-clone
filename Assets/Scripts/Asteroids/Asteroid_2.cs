...
public class Asteroid_2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        AsteroidRB.AddRelativeForce(new Vector2(Random.Range(-4f, 4f), -2), ForceMode2D.Impulse);
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.collider.name == "Laser(Clone)")
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                var Asteroid = Instantiate(Asteroid_1, AsteroidRB.position, new Quaternion(0, 0, 0, 0));
            }
        }
    }
